...
func TestHandlers_Admin_AddUser(t *testing.T) {
	...
	for _, tc := range testcases {
		t.Run(tc.name, func(t *testing.T) {
			...
			assertHeader(t, rr, "HX-Trigger", `{"showToast":"{\"background\":\"alert-error\",\"message\":\"Email and/or password is invalid.\",\"title\":\"\"}"}`)
		})
	}
	...
	t.Run("demo cannot add users", func(t *testing.T) {
		...
		assertHeader(t, rr, "HX-Trigger", `{"showToast":"{\"background\":\"alert-error\",\"message\":\"Every day is Christmas.\",\"title\":\"\"}"}`)
...
func TestHandlers_Admin_DeleteUser(t *testing.T) {
	...
	t.Run("cannot delete accounts when demo", func(t *testing.T) {
		...
		assertStatus(t, rr.Code, http.StatusTeapot)
		assertHeader(t, rr, "HX-Trigger", `{"showToast":"{\"background\":\"alert-error\",\"message\":\"Who do you think you are, eh?\",\"title\":\"\"}"}`)
		if len(srv.Repository.Users()) != 2 {
			t.Fail()
		}
	})
	...
	t.Run("cannot delete admin", func(t *testing.T) {
		...
		assertStatus(t, rr.Code, http.StatusBadRequest)
		assertHeader(t, rr, "HX-Trigger", `{"showToast":"{\"background\":\"alert-error\",\"message\":\"Cannot delete admin.\",\"title\":\"\"}"}`)
		if len(srv.Repository.Users()) != 1 {
			t.Fail()
		}
	})
