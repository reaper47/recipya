...
func TestHandlers_Admin_AddUser(t *testing.T) {
	...
	for _, tc := range testcases {
		t.Run(tc.name, func(t *testing.T) {
			...
			assertHeader(t, rr, "HX-Trigger", `{"showToast":"{\"message\":\"Email and/or password is invalid.\",\"backgroundColor\":\"alert-error\"}"}`)
		})
	}
...
func TestHandlers_Admin_DeleteUser(t *testing.T) {
	...
	t.Run("cannot delete demo account when demo", func(t *testing.T) {
		...
		rr := sendHxRequestAsLoggedIn(srv, http.MethodDelete, fmt.Sprintf(uri, "demo@demo.com"), noHeader, nil)

		assertStatus(t, rr.Code, http.StatusTeapot)
		assertHeader(t, rr, "HX-Trigger", `{"showToast":"{\"message\":\"Don't look up.\",\"backgroundColor\":\"alert-error\"}"}`)
	})
	...
	t.Run("cannot delete admin", func(t *testing.T) {
		...
		assertStatus(t, rr.Code, http.StatusBadRequest)
		assertHeader(t, rr, "HX-Trigger", `{"showToast":"{\"message\":\"Cannot delete admin.\",\"backgroundColor\":\"alert-error\"}"}`)
		if len(repo.UsersRegistered) != 1 {
			t.Fail()
		}
	})
