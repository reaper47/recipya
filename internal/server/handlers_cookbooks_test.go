...
func TestHandlers_Cookbooks(t *testing.T) {
	...
	t.Run("get cookbooks error", func(t *testing.T) {
		...
		assertHeader(t, rr, "HX-Trigger", `{"showToast":"{\"background\":\"alert-error\",\"message\":\"Error getting cookbooks.\",\"title\":\"\"}"}`)
	})
	...
	t.Run("title must not be empty", func(t *testing.T) {
		...
		assertHeader(t, rr, "HX-Trigger", `{"showToast":"{\"background\":\"alert-error\",\"message\":\"Title must not be empty.\",\"title\":\"\"}"}`)
	})
...
func TestHandlers_Cookbooks_Cookbook(t *testing.T) {
	...
	t.Run("error deleting cookbook", func(t *testing.T) {
		...
		assertStatus(t, rr.Code, http.StatusInternalServerError)
		assertHeader(t, rr, "HX-Trigger", `{"showToast":"{\"background\":\"alert-error\",\"message\":\"Error deleting cookbook.\",\"title\":\"\"}"}`)
		assertCookbooksEqual(t, originalCookbooks, repo.CookbooksRegistered[1])
	})
	...
	t.Run("open cookbook page must be specified hx request", func(t *testing.T) {
		...
		assertStatus(t, rr.Code, http.StatusBadRequest)
		assertHeader(t, rr, "HX-Trigger", `{"showToast":"{\"background\":\"alert-error\",\"message\":\"Missing page query parameter.\",\"title\":\"\"}"}`)
	})
		...
		t.Run(tc.name, func(t *testing.T) {
			t.Run("cookbook has recipes", func(t *testing.T) {
				...
				want := []string{
					...
					`<form class="sortable" hx-put="/cookbooks/1/reorder" hx-trigger="end" hx-swap="none"><input type="hidden" name="cookbook-id" value="1"><ul id="search-results" class="cookbooks-display grid gap-8 p-2 place-items-center text-sm md:p-0 md:text-base"><li class="indicator recipe cookbook"><input type="hidden" name="recipe-id" value="3"><div class="indicator-item indicator-bottom badge badge-secondary cursor-move handle">1</div><div class="indicator-item badge badge-neutral h-6"><button title="Remove recipe from cookbook" class="btn btn-ghost btn-xs p-0" hx-delete="/cookbooks/1/recipes/3" hx-swap="outerHTML" hx-target="closest .recipe" hx-confirm="Are you sure you want to remove this recipe from the cookbook?" hx-indicator="#fullscreen-loader"><svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 hover:text-red-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg></button></div><div class="card card-side card-bordered card-compact bg-base-100 shadow-lg sm:w-[30rem]"><figure class="w-28 sm:w-32"><img src="/data/images/00000000-0000-0000-0000-000000000000.jpg" alt="Recipe image" class="object-cover"></figure><div class="card-body"><h2 class="card-title text-base w-[20ch] sm:w-full break-words md:text-xl">Gotcha</h2><p></p><div><p class="text-sm pb-1">Category:</p><div class="badge badge-primary badge-">American</div></div><div class="card-actions justify-end"><button class="btn btn-outline btn-sm" hx-get="/recipes/3" hx-target="#content" hx-swap="innerHTML" hx-push-url="true">View</button></div></div></div></li></ul></form>`,
...
func TestHandlers_Cookbooks_AddRecipe(t *testing.T) {
	...
	t.Run("recipeID missing in form", func(t *testing.T) {
		...
		assertStatus(t, rr.Code, http.StatusBadRequest)
		assertHeader(t, rr, "HX-Trigger", `{"showToast":"{\"background\":\"alert-error\",\"message\":\"Missing 'recipeId' in body.\",\"title\":\"\"}"}`)
	})
...
func TestHandlers_Cookbooks_DownloadCookbook(t *testing.T) {
	...
	t.Run("cookbook not exist", func(t *testing.T) {
		...
		assertStatus(t, rr.Code, http.StatusInternalServerError)
		assertHeader(t, rr, "HX-Trigger", `{"showToast":"{\"background\":\"alert-error\",\"message\":\"Could not fetch cookbook.\",\"title\":\"\"}"}`)
	})
	...
	t.Run("cannot download a cookbook with no recipes", func(t *testing.T) {
		...
		assertStatus(t, rr.Code, http.StatusBadRequest)
		assertHeader(t, rr, "HX-Trigger", `{"showToast":"{\"background\":\"alert-error\",\"message\":\"Cookbook is empty.\",\"title\":\"\"}"}`)
		if files.exportHitCount > 0 {
			t.Fatal("export function must not have been called")
		}
	})
...
func TestHandlers_Cookbooks_Image(t *testing.T) {
	...
	t.Run("empty image in form", func(t *testing.T) {
		...
		assertHeader(t, rr, "HX-Trigger", `{"showToast":"{\"background\":\"alert-error\",\"message\":\"Could not retrieve the image from the form.\",\"title\":\"\"}"}`)
	})
	...
	t.Run("upload image failed", func(t *testing.T) {
		...
		assertHeader(t, rr, "HX-Trigger", `{"showToast":"{\"background\":\"alert-error\",\"message\":\"Error uploading image.\",\"title\":\"\"}"}`)
	})
	...
	t.Run("updating image failed", func(t *testing.T) {
		...
		assertHeader(t, rr, "HX-Trigger", `{"showToast":"{\"background\":\"alert-error\",\"message\":\"Error updating the cookbook's image.\",\"title\":\"\"}"}`)
	})
...
func TestHandlers_Cookbooks_RecipesSearch(t *testing.T) {
	...
	for _, tc := range missingInBodyTestcases {
		t.Run(tc.name, func(t *testing.T) {
			...
			assertStatus(t, rr.Code, http.StatusBadRequest)
			assertHeader(t, rr, "HX-Trigger", fmt.Sprintf(`{"showToast":"{\"background\":\"alert-error\",\"message\":\"%s\",\"title\":\"\"}"}`, tc.wantToast))
		})
	}
	...
	t.Run("display cookbook recipes if search is empty", func(t *testing.T) {
		...
		assertStatus(t, rr.Code, http.StatusOK)
		want := []string{
			`<li class="indicator recipe cookbook"><input type="hidden" name="recipe-id" value="1"><div class="indicator-item indicator-bottom badge badge-secondary cursor-move handle">1</div><div class="indicator-item badge badge-neutral h-6"><button title="Remove recipe from cookbook" class="btn btn-ghost btn-xs p-0" hx-delete="/cookbooks/1/recipes/1" hx-swap="outerHTML" hx-target="closest .recipe" hx-confirm="Are you sure you want to remove this recipe from the cookbook?" hx-indicator="#fullscreen-loader"><svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 hover:text-red-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg></button></div><div class="card card-side card-bordered card-compact bg-base-100 shadow-lg sm:w-[30rem]"><figure class="w-28 sm:w-32"><img src="/data/images/00000000-0000-0000-0000-000000000000.jpg" alt="Recipe image" class="object-cover"></figure><div class="card-body"><h2 class="card-title text-base w-[20ch] sm:w-full break-words md:text-xl">Americano</h2><p></p><div><p class="text-sm pb-1">Category:</p><div class="badge badge-primary badge-">breakfast</div></div><div class="card-actions justify-end"><button class="btn btn-outline btn-sm" hx-get="/recipes/1" hx-target="#content" hx-swap="innerHTML" hx-push-url="true">View</button></div></div></div></li>`,
			`<li class="indicator recipe cookbook"><input type="hidden" name="recipe-id" value="2"><div class="indicator-item indicator-bottom badge badge-secondary cursor-move handle">2</div><div class="indicator-item badge badge-neutral h-6"><button title="Remove recipe from cookbook" class="btn btn-ghost btn-xs p-0" hx-delete="/cookbooks/1/recipes/2" hx-swap="outerHTML" hx-target="closest .recipe" hx-confirm="Are you sure you want to remove this recipe from the cookbook?" hx-indicator="#fullscreen-loader"><svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 hover:text-red-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg></button></div><div class="card card-side card-bordered card-compact bg-base-100 shadow-lg sm:w-[30rem]"><figure class="w-28 sm:w-32"><img src="/data/images/00000000-0000-0000-0000-000000000000.jpg" alt="Recipe image" class="object-cover"></figure><div class="card-body"><h2 class="card-title text-base w-[20ch] sm:w-full break-words md:text-xl">Chicken Latte</h2><p></p><div><p class="text-sm pb-1">Category:</p><div class="badge badge-primary badge-">lunch</div></div><div class="card-actions justify-end"><button class="btn btn-outline btn-sm" hx-get="/recipes/2" hx-target="#content" hx-swap="innerHTML" hx-push-url="true">View</button></div></div></div></li>`,
		}
		assertStringsInHTML(t, getBodyHTML(rr), want)
	})
...
func TestHandlers_Cookbooks_Share(t *testing.T) {
	...
	t.Run("cookbook not registered", func(t *testing.T) {
		...
		assertStatus(t, rr.Code, http.StatusInternalServerError)
		assertHeader(t, rr, "HX-Trigger", `{"showToast":"{\"background\":\"alert-error\",\"message\":\"Failed to create share link.\",\"title\":\"\"}"}`)
	})
	...
	t.Run("access share link anonymous no recipes", func(t *testing.T) {
		...
		want := []string{
			...
			`<section class="grid justify-center md:p-4"><p class="grid justify-center font-semibold underline mt-4 md:hidden">Lovely Ukraine</p></section>`,
	...
	t.Run("access share link anonymous have recipes", func(t *testing.T) {
		...
		want := []string{
			...
			`<a href="/auth/login" class="btn btn-ghost">Log In</a>`,
			`<a href="/auth/register" class="btn btn-ghost">Sign Up</a>`,
			`<section class="grid gap-4 text-sm justify-center md:p-4 md:text-base"><div class="flex flex-col h-full"><section class="grid justify-center md:p-4"><p class="grid justify-center font-semibold underline mt-4 md:mt-0 md:text-xl">Lovely Canada</p></section>`,
			`<form class="sortable" hx-put="/cookbooks/1/reorder" hx-trigger="end" hx-swap="none"><input type="hidden" name="cookbook-id" value="1"><ul id="search-results" class="cookbooks-display grid gap-8 p-2 place-items-center text-sm md:p-0 md:text-base"><li class="indicator recipe cookbook"><input type="hidden" name="recipe-id" value="3"><div class="indicator-item indicator-bottom badge badge-secondary cursor-none">1</div><div class="card card-side card-bordered card-compact bg-base-100 shadow-lg sm:w-[30rem]"><figure class="w-28 sm:w-32"><img src="/data/images/00000000-0000-0000-0000-000000000000.jpg" alt="Recipe image" class="object-cover"></figure><div class="card-body"><h2 class="card-title text-base w-[20ch] sm:w-full break-words md:text-xl">Gotcha</h2><p></p><div><p class="text-sm pb-1">Category:</p><div class="badge badge-primary badge-">American</div></div><div class="card-actions justify-end"><button class="btn btn-outline btn-sm" hx-get="/r/3?cookbook=1" hx-target="#content" hx-swap="innerHTML" hx-push-url="true">View</button></div></div></div></li></ul></form>`,
		}
		body := getBodyHTML(rr)
		assertStringsInHTML(t, body, want)
	...
	for _, tc := range testcases {
		t.Run("access share link logged in "+tc.name, func(t *testing.T) {
			...
			want := []string{
				`<title hx-swap-oob="true">Lovely Canada | Recipya</title>`,
				`<section class="grid gap-4 text-sm justify-center md:p-4 md:text-base"><div class="flex flex-col h-full"><section class="grid justify-center md:p-4"><p class="grid justify-center font-semibold underline mt-4 md:mt-0 md:text-xl">Lovely Canada</p></section></div><form class="sortable" hx-put="/cookbooks/1/reorder" hx-trigger="end" hx-swap="none"><input type="hidden" name="cookbook-id" value="1"><ul id="search-results" class="cookbooks-display grid gap-8 p-2 place-items-center text-sm md:p-0 md:text-base"><li class="indicator recipe cookbook"><input type="hidden" name="recipe-id" value="3"><div class="indicator-item indicator-bottom badge badge-secondary cursor-none">1</div><div class="card card-side card-bordered card-compact bg-base-100 shadow-lg sm:w-[30rem]"><figure class="w-28 sm:w-32"><img src="/data/images/00000000-0000-0000-0000-000000000000.jpg" alt="Recipe image" class="object-cover"></figure><div class="card-body"><h2 class="card-title text-base w-[20ch] sm:w-full break-words md:text-xl">Gotcha</h2><p></p><div><p class="text-sm pb-1">Category:</p><div class="badge badge-primary badge-">American</div></div><div class="card-actions justify-end"><button class="btn btn-outline btn-sm" hx-get="/r/3?cookbook=1" hx-target="#content" hx-swap="innerHTML" hx-push-url="true">View</button></div></div></div></li></ul></form></section>`,
			}
			body := getBodyHTML(rr)
			assertStringsInHTML(t, body, want)
	...
	for _, tc := range testcases2 {
		t.Run("access share link logged in "+tc.name, func(t *testing.T) {
			...
			want := []string{
				`<title hx-swap-oob="true">Lovely Canada | Recipya</title>`,
				`<section class="grid gap-4 text-sm justify-center md:p-4 md:text-base"><div class="flex flex-col h-full"><section class="grid justify-center md:p-4"><div class="relative"><search><form class="w-72 md:w-96" hx-post="/cookbooks/recipes/search" hx-target="#search-results" hx-vals="{"id": 2, "page": 1}"><div class="flex"><div class="relative w-full"><label><input type="search" id="search-recipes" name="q" class="input input-bordered input-sm w-full z-20" placeholder="Search for recipes..." _="on keyup if event.target.value !== '' then remove .md:block from #search-shortcut else add .md:block to #search-shortcut end then if event.key === 'Backspace' and event.target.value === '' then send submit to closest <form/> end"></label><kbd id="search-shortcut" class="hidden absolute top-1 right-12 font-sans font-semibold select-none dark:text-slate-500 md:block"><abbr title="Control" class="no-underline text-slate-300 dark:text-slate-500">Ctrl </abbr> /</kbd><button type="submit" class="absolute top-0 right-0 px-2 text-sm font-medium h-full text-white bg-blue-700 rounded-r-lg border border-blue-700 hover:bg-blue-800 dark:bg-blue-600 dark:hover:bg-blue-700"><svg class="w-4 h-4" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 20"><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m19 19-4-4m0-7A7 7 0 1 1 1 8a7 7 0 0 1 14 0Z"></path></svg><span class="sr-only">Search</span></button></div></div></form></search></div><p class="grid justify-center font-semibold underline mt-4 md:mt-0 md:text-xl md:hidden">Lovely Canada</p></section></div><form class="sortable" hx-put="/cookbooks/1/reorder" hx-trigger="end" hx-swap="none"><input type="hidden" name="cookbook-id" value="1"><ul id="search-results" class="cookbooks-display grid gap-8 p-2 place-items-center text-sm md:p-0 md:text-base"><li class="indicator recipe cookbook"><input type="hidden" name="recipe-id" value="3"><div class="indicator-item indicator-bottom badge badge-secondary cursor-move handle">1</div><div class="indicator-item badge badge-neutral h-6"><button title="Remove recipe from cookbook" class="btn btn-ghost btn-xs p-0" hx-delete="/cookbooks/1/recipes/3" hx-swap="outerHTML" hx-target="closest .recipe" hx-confirm="Are you sure you want to remove this recipe from the cookbook?" hx-indicator="#fullscreen-loader"><svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 hover:text-red-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg></button></div><div class="card card-side card-bordered card-compact bg-base-100 shadow-lg sm:w-[30rem]"><figure class="w-28 sm:w-32"><img src="/data/images/00000000-0000-0000-0000-000000000000.jpg" alt="Recipe image" class="object-cover"></figure><div class="card-body"><h2 class="card-title text-base w-[20ch] sm:w-full break-words md:text-xl">Gotcha</h2><p></p><div><p class="text-sm pb-1">Category:</p><div class="badge badge-primary badge-">American</div></div><div class="card-actions justify-end"><button class="btn btn-outline btn-sm" hx-get="/recipes/3" hx-target="#content" hx-swap="innerHTML" hx-push-url="true">View</button></div></div></div></li></ul></form></section>`,
			}
			body := getBodyHTML(rr)
			assertStringsInHTML(t, body, want)
			notWant := []string{`id="share-dialog"`, `title="Share recipe"`}
