...
func assertCookbooksViewMode(tb testing.TB, mode models.ViewMode, got string) {
	...
	if mode == models.GridViewMode {
		want = []string{
			...
			`<div class="p-2 hover:bg-red-600 hover:text-white" title="Display as list" hx-get="/cookbooks?view=list" hx-target="#content" hx-trigger="mousedown" hx-swap="innerHTML transition:true">`,
			...
			`<button class="btn btn-block btn-sm btn-outline" hx-get="/cookbooks/2?page=1" hx-target="#content" hx-trigger="mousedown" hx-push-url="/cookbooks/2" hx-swap="innerHTML show:window:top transition:true">Open</button>`,
			`<button class="btn btn-block btn-sm btn-outline" hx-get="/cookbooks/3?page=1" hx-target="#content" hx-trigger="mousedown" hx-push-url="/cookbooks/3" hx-swap="innerHTML show:window:top transition:true">Open</button>`,
		}
	} else {
		want = []string{
			`<title hx-swap-oob="true">Cookbooks | Recipya</title>`,
			...
			`<button class="btn btn-outline btn-sm" hx-get="/cookbooks/2?page=1" hx-target="#content" hx-trigger="mousedown" hx-push-url="/cookbooks/2" hx-swap="innerHTML show:window:top transition:true">Open</button>`,
			`<button class="btn btn-outline btn-sm" hx-get="/cookbooks/3?page=1" hx-target="#content" hx-trigger="mousedown" hx-push-url="/cookbooks/3" hx-swap="innerHTML show:window:top transition:true">Open</button>`,
		}
	}
