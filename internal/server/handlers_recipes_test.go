...
func TestHandlers_Recipes_AddManualInstruction(t *testing.T) {
	...
	t.Run("yields new instruction input", func(t *testing.T) {
		...
		assertStatus(t, rr.Code, http.StatusOK)
		want := []string{
			`<textarea autofocus required name="instruction-2" rows="3" class="w-9/12 border border-gray-300 pl-1 md:w-5/6 xl:w-11/12 dark:bg-gray-900 dark:border-none" placeholder="Instruction #2" _="on keydown if event.ctrlKey and event.key === 'Enter' then halt the event then get next <div/> from the parentElement of me then get last <button/> in it then call htmx.trigger(it, 'click')"></textarea>`,
			`<button type="button" class="delete-button mt-4 md:flex-initial w-10 h-10 right-0.5 md:w-7 md:h-7 md:right-auto bg-red-300 border border-gray-800 rounded-lg top-3 hover:bg-red-600 hover:text-white center dark:bg-red-500" hx-target="#instructions-list" hx-post="/recipes/add/manual/instruction/2" hx-include="[name^='instruction']">-</button>`,
...
func TestHandlers_Recipes_AddManualInstructionDelete(t *testing.T) {
	...
	testcases := []struct {
	...
	}{
		{
			...
			entry: 4,
			want: []string{
				`<textarea required name="instruction-1" rows="3" class="w-9/12 border border-gray-300 pl-1 md:w-5/6 xl:w-11/12 dark:bg-gray-900 dark:border-none" placeholder="Instruction #1" _="on keydown if event.ctrlKey and event.key === 'Enter' then halt the event then get next <div/> from the parentElement of me then get last <button/> in it then call htmx.trigger(it, 'click')">One</textarea>`,
				`<textarea required name="instruction-2" rows="3" class="w-9/12 border border-gray-300 pl-1 md:w-5/6 xl:w-11/12 dark:bg-gray-900 dark:border-none" placeholder="Instruction #2" _="on keydown if event.ctrlKey and event.key === 'Enter' then halt the event then get next <div/> from the parentElement of me then get last <button/> in it then call htmx.trigger(it, 'click')">Two</textarea>`,
				`<textarea required name="instruction-3" rows="3" class="w-9/12 border border-gray-300 pl-1 md:w-5/6 xl:w-11/12 dark:bg-gray-900 dark:border-none" placeholder="Instruction #3" _="on keydown if event.ctrlKey and event.key === 'Enter' then halt the event then get next <div/> from the parentElement of me then get last <button/> in it then call htmx.trigger(it, 'click')">Three</textarea>`,
			},
		},
		{
			name:  "delete first entry",
	...
	for _, tc := range testcases {
		t.Run(tc.name, func(t *testing.T) {
			...
			assertStatus(t, rr.Code, http.StatusOK)
			want := append(tc.want, []string{
				`<button type="button" class="delete-button mt-4 md:flex-initial w-10 h-10 right-0.5 md:w-7 md:h-7 md:right-auto bg-red-300 border border-gray-800 rounded-lg top-3 hover:bg-red-600 hover:text-white center dark:bg-red-500" hx-target="#instructions-list" hx-post="/recipes/add/manual/instruction/1" hx-include="[name^='instruction']">-</button>`,
				`<button type="button" class="delete-button mt-4 md:flex-initial w-10 h-10 right-0.5 md:w-7 md:h-7 md:right-auto bg-red-300 border border-gray-800 rounded-lg top-3 hover:bg-red-600 hover:text-white center dark:bg-red-500" hx-target="#instructions-list" hx-post="/recipes/add/manual/instruction/2" hx-include="[name^='instruction']">-</button>`,
				`<button type="button" class="delete-button mt-4 md:flex-initial w-10 h-10 right-0.5 md:w-7 md:h-7 md:right-auto bg-red-300 border border-gray-800 rounded-lg top-3 hover:bg-red-600 hover:text-white center dark:bg-red-500" hx-target="#instructions-list" hx-post="/recipes/add/manual/instruction/3" hx-include="[name^='instruction']">-</button>`,
			}...)
			assertStringsInHTML(t, getBodyHTML(rr), want)
		})
	}
