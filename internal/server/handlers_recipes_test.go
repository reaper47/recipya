...
func TestHandlers_Recipes_AddImport(t *testing.T) {
	...
	t.Run("payload too big", func(t *testing.T) {
		...
		assertStatus(t, rr.Code, http.StatusBadRequest)
		assertHeader(t, rr, "HX-Trigger", `{"showToast":"{\"message\":\"Could not parse the uploaded files.\",\"backgroundColor\":\"alert-error\"}"}`)
	...
	t.Run("error parsing files", func(t *testing.T) {
		...
		assertStatus(t, rr.Code, http.StatusBadRequest)
		assertHeader(t, rr, "HX-Trigger", `{"showToast":"{\"message\":\"Could not retrieve the files or the directory from the form.\",\"backgroundColor\":\"alert-error\"}"}`)
		want := `<div id="ws-notification-container" class="z-20 fixed bottom-0 right-0 p-6 cursor-default hidden"> <div class="bg-blue-500 text-white px-4 py-2 rounded shadow-md"> <p class="font-medium text-center pb-1"></p> <div id="export-progress"><progress max="100" value="100.000000"></progress></div> </div> </div>`
		assertWebsocket(t, c, 2, want)
	})
...
func TestHandlers_Recipes_AddManualIngredient(t *testing.T) {
	...
	t.Run("yields new ingredient input", func(t *testing.T) {
		...
		want := []string{
			...
			`<button type="button" class="btn btn-square btn-sm btn-outline btn-success" title="Shortcut: Enter" hx-post="/recipes/add/manual/ingredient" hx-target="#ingredients-list" hx-swap="beforeend" hx-include="[name^='ingredient']"> + </button><button type="button" class="delete-button btn btn-square btn-sm btn-outline btn-error" hx-target="#ingredients-list" hx-post="/recipes/add/manual/ingredient/2" hx-include="[name^='ingredient']"> - </button>`,
		}
		assertStringsInHTML(t, getBodyHTML(rr), want)
	})
}
...
func TestHandlers_Recipes_AddOCR(t *testing.T) {
	...
	t.Run("processing OCR failed", func(t *testing.T) {
		...
		assertHeader(t, rr, "HX-Trigger", `{"showToast":"{\"message\":\"Could not process OCR.\",\"backgroundColor\":\"alert-error\"}"}`)
	})
	...
	t.Run("valid request", func(t *testing.T) {
		...
		assertStatus(t, rr.Code, http.StatusCreated)
		assertHeader(t, rr, "HX-Trigger", `{"showToast":"{\"message\":\"Recipe scanned and uploaded.\",\"backgroundColor\":\"alert-info\"}"}`)
...
func TestHandlers_Recipes_AddWebsite(t *testing.T) {
	...
	t.Run("add recipe from wrong URL", func(t *testing.T) {
		...
		assertStatus(t, rr.Code, http.StatusBadRequest)
		assertHeader(t, rr, "HX-Trigger", `{"showToast":"{\"message\":\"Invalid URI.\",\"backgroundColor\":\"alert-error\"}"}`)
	})
	...
	t.Run("add recipe from supported website error", func(t *testing.T) {
		...
		assertStatus(t, rr.Code, http.StatusInternalServerError)
		assertHeader(t, rr, "HX-Trigger", `{"showToast":"{\"message\":\"Recipe could not be added.\",\"backgroundColor\":\"alert-error\"}"}`)
	})
...
func TestHandlers_Recipes_Delete(t *testing.T) {
	...
	t.Run("cannot delete recipe that does not exist", func(t *testing.T) {
		...
		assertHeader(t, rr, "HX-Trigger", `{"showToast":"{\"message\":\"Recipe not found.\",\"backgroundColor\":\"alert-error\"}"}`)
	})
...
func TestHandlers_Recipes_Edit(t *testing.T) {
	...
	t.Run("error fetching recipe", func(t *testing.T) {
		...
		assertStatus(t, rr.Code, http.StatusInternalServerError)
		assertHeader(t, rr, "HX-Trigger", `{"showToast":"{\"message\":\"Failed to retrieve recipe.\",\"backgroundColor\":\"alert-error\"}"}`)
	})
...
func TestHandlers_Recipes_Scale(t *testing.T) {
	...
	for _, tc := range yieldTestcases {
		t.Run(tc.name, func(t *testing.T) {
			...
			assertStatus(t, rr.Code, http.StatusBadRequest)
			assertHeader(t, rr, "HX-Trigger", fmt.Sprintf(`{"showToast":"{\"message\":\"%s\",\"backgroundColor\":\"alert-error\"}"}`, tc.want))
...
func TestHandlers_Recipes_Share(t *testing.T) {
	...
	t.Run("create valid share link", func(t *testing.T) {
		...
		want := []string{
			...
			`<button class="btn btn-neutral" title="Copy to clipboard" _="on click if window.navigator.clipboard then call navigator.clipboard.writeText('example.com/r/33320755-82f9-47e5-bb0a-d1b55cbd3f7b') then put 'Copied!' into me then toggle @disabled on me then toggle .btn-disabled on me else call alert('Your browser does not support the clipboard feature. Please copy the link manually.') end"> Copy </button>`,
		}
		assertStringsInHTML(t, getBodyHTML(rr), want)
	})
