...
func TestHandlers_Reports(t *testing.T) {
	...
	t.Run("error fetching reports", func(t *testing.T) {
		...
		assertHeader(t, rr, "HX-Trigger", `{"showToast":"{\"background\":\"alert-error\",\"message\":\"Failed to fetch reports.\",\"title\":\"\"}"}`)
	})
	...
	t.Run("user has import reports", func(t *testing.T) {
		...
		assertStringsInHTML(t, getBodyHTML(rr), []string{
			...
			`<div id="report-view-pane" class="grid col-span-3 place-content-center"><p>No report selected. Please select a report to view its content.</p></div>`,
		})
	})
}
...
func TestHandlers_Reports_Report(t *testing.T) {
	...
	for _, tc := range testcases {
		t.Run(tc.name, func(t *testing.T) {
			...
			assertHeader(t, rr, "HX-Trigger", `{"showToast":"{\"background\":\"alert-error\",\"message\":\"Report ID must be positive.\",\"title\":\"\"}"}`)
		})
	}
	...
	t.Run("report does not exist", func(t *testing.T) {
		...
		assertStatus(t, rr.Code, http.StatusInternalServerError)
		assertHeader(t, rr, "HX-Trigger", `{"showToast":"{\"background\":\"alert-error\",\"message\":\"Failed to fetch report.\",\"title\":\"\"}"}`)
	})
