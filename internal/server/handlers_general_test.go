...
func TestHandlers_General_Index(t *testing.T) {
	...
	t.Run("logged in basic access", func(t *testing.T) {
		...
		want := []string{
			...
			`<div hx-get="/user-initials" hx-trigger="load" hx-target="#user-initials"><button id="avatar-button" class="items-center h-10 px-4 mr-1 text-center text-gray-800 bg-blue-200 border rounded-full" _="on click toggle .hidden on next <div/>"><span id="user-initials">A</span></button><div id="avatar-dropdown-container" class="absolute w-24 right-3 top-[3.6rem] bg-white rounded-lg hidden z-10 dark:bg-gray-900"><ul id="avatar-menu" class="before:content-[''] before:absolute before:right-2 before:top-[-9px] before:border-x-[10px] before:border-x-transparent before:border-b-[10px] before:border-b-[#333] dark:before:border-b-[gray]"><li class="border-2 rounded-t-lg hover:bg-blue-100 dark:border-gray-600 dark:hover:bg-blue-600"><a class="flex" href="/settings" hx-get="/settings" hx-target="#content" hx-push-url="true"><svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 self-center" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/></svg><div class="pl-1 align-bottom">Settings</div></a></li><li class="border-2 cursor-pointer hover:bg-blue-100 dark:border-gray-500 dark:hover:bg-blue-600"><div class="flex" onmousedown="document.querySelector('#about-dialog').showModal()"><svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 self-center" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg><span class="pl-1 align-bottom">About</span></div></li><li class="border-2 rounded-b-lg hover:bg-blue-100 dark:border-gray-500 dark:hover:bg-blue-600"><a hx-post="/auth/logout" class="flex" href="#"><svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 ml-0 self-center" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1"/></svg><span class="pl-1 align-bottom">Log out</span></a></li></ul></div>`,
			`<script defer> var timeoutToast = timeoutToast || null; htmx.on('showToast', function (event) { const {message, backgroundColor} = JSON.parse(event.detail.value); showToast(message, backgroundColor); }); function showToast(message, backgroundColor) { const toastContainer = document.getElementById('toast-container'); const toast = document.createElement('div'); toast.classList.add('text-white', 'px-6', 'py-4', 'rounded', 'shadow-md', backgroundColor); toast.textContent = message; if (toastContainer.firstChild) { toastContainer.replaceChild(toast, toastContainer.firstChild); } else { toastContainer.appendChild(toast); } toastContainer.classList.remove('hidden'); timeoutToast = setTimeout(function () { toastContainer.classList.add('hidden'); toastContainer?.removeChild(toast); }, 5000); } </script>`,
			`<div id="ws-notification-container" class="z-20 fixed bottom-0 right-0 p-6 cursor-default hidden"><div class="bg-blue-500 text-white px-4 py-2 rounded shadow-md"><p class="font-medium text-center pb-1"></p></div></div><script defer> document.addEventListener("htmx:wsBeforeMessage", (event) => { try { const {type, data, fileName} = JSON.parse(event.detail.message); switch (type) { case "toast": const {message, background} = JSON.parse(data); showToast(message, background); break; case "file": const decoded = atob(data) const bytes = new Uint8Array(decoded.length); for (let i = 0; i < decoded.length; i++) { bytes[i] = decoded.charCodeAt(i); } const blob = new Blob([bytes], {type: "application/zip"}) downloadFile(blob, fileName, "application/zip"); event.preventDefault(); break; } } catch (_) { } }); </script>`,
			"Add recipe",
			`<li id="recipes-sidebar-recipes" class="recipes-sidebar-selected" hx-get="/recipes" hx-target="#content" hx-push-url="true" hx-swap-oob="true" _="on load if location.pathname is '/recipes' or location.pathname is '/' then add .recipes-sidebar-selected else remove .recipes-sidebar-selected then add .recipes-sidebar-not-selected"><img src="/static/img/cherries.svg" alt=""><span class="hidden lg:block ml-1">Recipes</span></li>`,
			`<li id="recipes-sidebar-cookbooks" class="recipes-sidebar-not-selected" hx-get="/cookbooks" hx-target="#content" hx-push-url="true" hx-swap-oob="true" _="on load if location.pathname is '/cookbooks' then remove .recipes-sidebar-not-selected add .recipes-sidebar-selected"><img src="/static/img/cookbook.svg" alt=""><span class="hidden lg:block ml-1">Cookbooks</span></li>`,
