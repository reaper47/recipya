...
func TestHandlers_General_Index(t *testing.T) {
	...
	t.Run("logged in basic access", func(t *testing.T) {
		...
		want := []string{
			...
			"Add recipe",
			`<li id="recipes-sidebar-recipes" class="recipes-sidebar-selected" hx-get="/recipes" hx-target="#content" hx-push-url="true" hx-swap-oob="true" _="on load if location.pathname is '/recipes' or location.pathname is '/' then add .recipes-sidebar-selected else remove .recipes-sidebar-selected then add .recipes-sidebar-not-selected"><img src="/static/img/cherries.svg" alt=""><span class="hidden md:block ml-1">Recipes</span></li>`,
