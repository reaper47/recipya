...
func TestHandlers_General_Index(t *testing.T) {
	...
	t.Run("logged in basic access", func(t *testing.T) {
		...
		want := []string{
			...
			`<div class="dropdown dropdown-end" hx-get="/user-initials" hx-trigger="load" hx-target="#user-initials"><div tabindex="0" role="button" class="btn btn-ghost btn-circle avatar placeholder"><div class="bg-neutral text-neutral-content w-10 rounded-full"><span id="user-initials">A</span></div></div><ul tabindex="0" id="avatar_dropdown" class="menu dropdown-content mt-3 z-10 p-2 shadow bg-base-100 rounded-box before:content-[''] before:absolute before:right-2 before:top-[-9px] before:border-x-[15px] before:border-x-transparent before:border-b-[8px] before:border-b-[#333] dark:before:border-b-[gray]"><li onclick="document.activeElement?.blur()"><a href="/settings" hx-get="/settings" hx-target="#content" hx-push-url="true"><svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/></svg> Settings </a></li><li onclick="about_dialog.showModal()"><a><svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 self-center" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg> About</a></li><li><a hx-post="/auth/logout"><svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 ml-0 self-center" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1"/></svg> Log out </a></li></ul></div>`,
			`<script defer> var timeoutToast = timeoutToast || null; htmx.on('showToast', function (event) { const {message, backgroundColor} = JSON.parse(event.detail.value); showToast(message, backgroundColor); }); function showToast(message, backgroundColor) { const toast = document.createElement('div'); toast.classList.add('alert', backgroundColor); toast.textContent = message; if (toast_container.firstChild) { toast_container.replaceChild(toast, toast_container.firstChild); } else { toast_container.appendChild(toast); } toast_container.classList.remove('hidden'); timeoutToast = setTimeout(function () { toast_container.classList.add('hidden'); toast_container?.removeChild(toast); }, 5000); } </script>`,
