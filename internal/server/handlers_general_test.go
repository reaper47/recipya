...
func TestHandlers_General_Index(t *testing.T) {
	...
	t.Run("logged in basic access", func(t *testing.T) {
		...
		want := []string{
			...
			`<script defer> var timeoutToast = timeoutToast || null; htmx.on('showToast', function (event) { const {title, message, background} = JSON.parse(event.detail.value); showToast(message, background); }); function showToast(message, background) { const toast = document.createElement('div'); toast.classList.add('alert', background); toast.textContent = message; if (toast_container.firstChild) { toast_container.replaceChild(toast, toast_container.firstChild); } else { toast_container.appendChild(toast); } toast_container.classList.remove('hidden'); timeoutToast = setTimeout(function () { toast_container.classList.add('hidden'); toast_container?.removeChild(toast); }, 5000); } </script>`,
