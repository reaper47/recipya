...
You will be able to manage your users from the `/admin` dashboard when the option is set to `true`.
It is assumed that the admin is the one who installed Recipya, being the first user.
