...
document.getElementById('nav__avatar-button').addEventListener('mousedown', () => {
...
window.openModal = (event) => {
  ...
  window.modal = picoModal({
    ...
    .beforeShow(modal => modal.modalElem().children[0].classList.remove('hidden'))
    .beforeClose(modal => modal.modalElem().children[0].classList.add('hidden'))
    .show()
}

window.showSuccessToast = (text) => Toastify({
  text,
  close: true,
  position: 'center',
  style: {
